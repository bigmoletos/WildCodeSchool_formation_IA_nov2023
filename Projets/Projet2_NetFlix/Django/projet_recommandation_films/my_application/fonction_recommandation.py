...
def recommandation_film(my_dataframe,X_reduced,nom_film):
    ...
    liste_colonnes_non_numeriques = df.select_dtypes(exclude=[np.number]).columns.tolist()
    ...
    knn_recommandation2 = NearestNeighbors(n_neighbors=20)
    ...
    distances, indices = knn_recommandation2.kneighbors([X_reduced[index_film]])
    ...
    unique_tconsts = [df.iloc[index_film]['tconst'] for index_film in indices_plat]
